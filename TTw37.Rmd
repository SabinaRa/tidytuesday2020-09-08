...
p1 <- p1 + annotate(geom = "text", x  =episodesmonth$Month, y = episodesmonth$Freq, label = episodesmonth$Freq, size = 4, color = "white", fontface = "bold") + annotate(geom = "text", x = episodesmonth$Month[episodesmonth$Freq== max(episodesmonth$Freq)], y = max(episodesmonth$Freq), label = max(episodesmonth$Freq), size = 4, color = "#ff4747", fontface = "bold") + annotate("text", label = "Friends episodes were mostly \naired during October", x = "Jul", y = 37) + geom_curve(x ="Aug", y = 40, xend = "Oct", yend = 40,  curvature = -.5, arrow = arrow(length = unit(2, "mm")))


ratingsmonth <-  friends_info %>% mutate( month = lubridate::month(air_date, label = TRUE)) %>% 
...
months <- factor(ratingsmonth$month, levels = month_levels)

p2 <- ggplot(ratingsmonth, aes(x=sort(months), y=imdb_rating)) + 
